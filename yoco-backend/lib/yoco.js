...
async function createCheckout({ amountInCents, description, metadata }) {
  ...
  if (secretKey.startsWith("pk_")) {
    throw new Error(
      "YOCO_SECRET_KEY must be a secret key (sk_test_... or sk_live_...), not a public key (pk_...). Create one in the Yoco portal under API keys."
    );
  }

  const siteUrl = (process.env.SITE_URL || "https://theshack.co.za").replace(
    /\/$/,
    ""
  );
  ...
  const body = {
    ...
    currency: "ZAR",
    description: description || "The Shack booking",
    metadata: metadata || {},
    successRedirectUrl: process.env.YOCO_SUCCESS_URL || `${siteUrl}/?payment=success`,
    cancelRedirectUrl: process.env.YOCO_CANCEL_URL || `${siteUrl}/?payment=cancel`,
    failureRedirectUrl: process.env.YOCO_FAILURE_URL || `${siteUrl}/?payment=failed`,
  };
